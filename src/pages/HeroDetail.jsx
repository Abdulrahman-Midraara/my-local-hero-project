...
import { fetchHero, addComment, thankHero } from "../store/slices/heroesSlice";

const HeroDetail = () => {
  const { id } = useParams();
  const dispatch = useDispatch();
  const { currentHero, comments, loading, error } = useSelector(
    (state) => state.heroes
  );
  ...
  const handleThank = async () => {
    ...
    if (result.meta.requestStatus === "fulfilled") {
      ...
      setThankError(
        "You have already thanked this hero. You cannot thank twice."
      );
    ...
    <div className="max-w-2xl mx-auto  p-6 mt-10 space-y-4">
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-lg shadow-md mt-10 p-0 overflow-hidden">
        ...
        <div className="relative">
          <img
            src={
              imageError || !currentHero.photo_url
        ...
        <div className="p-6">
          ...
          <div className="flex items-center gap-4 mb-6">
            <span className="text-pink-500 font-bold text-lg">
              {currentHero.thanks_count || 0}
            </span>
            <span className="text-gray-500 text-sm">
              people have thanked {currentHero.full_name}
            </span>
            <button
              ...
              {comments.length === 0 ? (
                ...
                comments.map((comment, i) => {
                  ...
                  return (
                    <div
                      key={i}
                      className="flex gap-3 p-4 bg-gray-50 rounded-lg border border-gray-100 hover:bg-gray-100 transition-colors"
                    >
